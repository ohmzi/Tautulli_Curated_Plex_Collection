#!/bin/bash
#
# Midnight Collection Organizer Runner
#
# This script runs the midnight_collection_organizer.py script with proper
# environment setup and error handling.
#
# Usage:
#   ./run_midnight_organizer.sh [options]
#
# Options:
...
ORGANIZER_SCRIPT="$SCRIPT_DIR/midnight_collection_organizer.py"
if [[ ! -f "$ORGANIZER_SCRIPT" ]]; then
    echo -e "${RED}Error: midnight_collection_organizer.py not found at: $ORGANIZER_SCRIPT${NC}"
    exit 1
fi

# Check if config.yaml exists
if [[ ! -f "$SCRIPT_DIR/config.yaml" ]]; then
    echo -e "${YELLOW}Warning: config.yaml not found at: $SCRIPT_DIR/config.yaml${NC}"
...
if [[ -n "$LOG_FILE" ]]; then
    ...
    LOG_PATH="$SCRIPT_DIR/logs/midnight_organizer_${TIMESTAMP}.log"
...
CMD="$PYTHON_CMD -u $ORGANIZER_SCRIPT"
if [[ -n "$DRY_RUN" ]]; then
    CMD="$CMD $DRY_RUN"
fi
if [[ -n "$VERBOSE" ]]; then
...
output_color "${BLUE}Midnight Collection Organizer${NC}"
output_color "${BLUE}========================================${NC}"
output ""
output_color "Script: ${GREEN}$ORGANIZER_SCRIPT${NC}"
output_color "Working directory: ${GREEN}$SCRIPT_DIR${NC}"
...
output_color "${BLUE}Starting organizer...${NC}"
output ""
