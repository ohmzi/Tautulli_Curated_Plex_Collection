...
Usage:
    python3 midnight_collection_organizer.py [--dry-run] [--verbose]
...
logger = setup_logger("midnight_collection_organizer")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Midnight Collection Organizer - Updates Plex collection during off-peak hours",
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("MIDNIGHT COLLECTION ORGANIZER START")
    logger.info("=" * 60)
    ...
    try:
        ...
        cfg = load_config()
        logger.info(f"  ✓ Config loaded from: {cfg.base_dir / 'config.yaml'}")
        ...
        if not points_path.exists():
            ...
            logger.info("MIDNIGHT COLLECTION ORGANIZER END (file not found)")
            return 1
        ...
        if not points_data:
            ...
            logger.info("MIDNIGHT COLLECTION ORGANIZER END (no points file)")
            return 0
        ...
        if not rating_keys:
            ...
            logger.info("MIDNIGHT COLLECTION ORGANIZER END (empty points)")
            return 0
        ...
        if not items:
            ...
            logger.info("MIDNIGHT COLLECTION ORGANIZER END (no valid items)")
            return 0
        ...
        # Final summary
        logger.info("=" * 60)
        logger.info("MIDNIGHT COLLECTION ORGANIZER SUMMARY")
        logger.info("=" * 60)
        ...
        logger.info("=" * 60)
        logger.info("MIDNIGHT COLLECTION ORGANIZER END OK")
        logger.info("=" * 60)
        return 0
        
    except KeyboardInterrupt:
        logger.warning("\nInterrupted by user")
        logger.info("MIDNIGHT COLLECTION ORGANIZER END (interrupted)")
        return 130
    except Exception as e:
        logger.exception("MIDNIGHT COLLECTION ORGANIZER END FAIL")
